...
export default {
  activate() {
    ...
    this.subscriptions.add(atom.config.observe(
      'linter-erlc.executablePath',
      (value) => { this.executablePath = value; },
    ));

    this.subscriptions.add(atom.config.observe(
      'linter-erlc.pa',
      (value) => { this.erlcPa = value; },
    ));

    this.subscriptions.add(atom.config.observe(
      'linter-erlc.includeDirs',
      (value) => { this.erlcIncludeDirs = value; },
    ));
  },
      ...
      lint: (textEditor) => {
        ...
        return helpers.exec(this.executablePath, args, { ignoreExitCode: true }).then((result) => {
          ...
          const toReturn = matches.reduce((results, match) => {
            ...
            let severity;
            if (excerpt.match(/Warning:/)) {
              excerpt = excerpt.replace(/Warning: (.+)/, '$1');
              severity = 'warning';
            } else {
              severity = 'error';
            }

            const beforeMatch = excerpt.match(/before: '?(.+?)'?/);
            const unUsedMatch = excerpt.match(/variable '(.+)' is unused/);
